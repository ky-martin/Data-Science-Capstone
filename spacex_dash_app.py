...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                ...
                                8000:'8000',10000:'10000'}, value=[min_payload, max_payload]),
...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        fig = px.pie(data_frame=filtered_df, values='class', title='Total Launches for Site')
        return fig
# TASK 4:
...
def get_scatter(entered_site, payload_slider):
    if entered_site == 'ALL':
        payload_data = spacex_df[(spacex_df['Payload Mass (kg)']>=payload_slider[0])
        &(spacex_df['Payload Mass (kg)']<=payload_slider[1])]
        splot = px.scatter(data_frame=payload_data, x="Payload Mass (kg)", y="class", 
        ...
        &(spacex_df['Payload Mass (kg)']<=payload_slider[1])]
        splot = px.scatter(data_frame=filtered_data, x="Payload Mass (kg)", y="class", 
        color="Booster Version Category")
        return splot
